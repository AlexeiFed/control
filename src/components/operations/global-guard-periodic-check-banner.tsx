...
const DISMISS_STORAGE_KEY = "guard-periodic-check-banner-dismiss";

export function GlobalGuardPeriodicCheckBanner() {
  ...
  useLayoutEffect(() => {
    ...
    if (!items?.length || !el || dismissed) {
      document.documentElement.style.setProperty("--periodic-check-banner-offset", "0px");
      return;
    }
    const sync = () => {
      const h = el.getBoundingClientRect().height;
      document.documentElement.style.setProperty("--periodic-check-banner-offset", `${Math.ceil(h + 16)}px`);
    };
    sync();
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch("/api/guards/periodic-check-reminders", {
  ...
  return (
    <div
      ref={bannerRef}
      className="pointer-events-auto fixed left-1/2 z-[99] max-h-[min(70vh,24rem)] w-[min(calc(100vw-2rem),26rem)] -translate-x-1/2 overflow-y-auto rounded-card border px-3 py-2 pr-8 text-xs shadow-glow"
      style={{
    ...
    >
      ...
      <div className="font-semibold pr-4" style={{ color: designTokens.color.accent.warning }}>
      ...
      <ul className="mt-2 list-none space-y-2 p-0">
        {items.map((item) => (
          <li key={item.guardId} className="leading-snug">
            <span className="font-medium">{item.guardName}</span>
            {item.reminderKind === "personal_card" ? (
              <>
                : ЛК от {item.passedOnDisplay}, пер. проверка не пройдена в срок (с {item.expiryDisplay})
              </>
            ) : (
              <>
